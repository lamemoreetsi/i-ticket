...
<head>
    ...
    <title>Ticket — Admin · HelpDesk</title>
    ...
    <style>
        ...
        .admin-layout { display:grid;grid-template-columns:1fr 300px;gap:1.25rem;align-items:start; }
        .admin-panel { background:#111827;border:1px solid #1e3a5f;border-radius:12px;overflow:hidden;position:sticky;top:74px; }
        .admin-panel-header { display:flex;align-items:center;gap:8px;padding:14px 18px;background:#0d1424;border-bottom:1px solid #1e3a5f;font-size:13px;font-weight:600;color:#94a3b8; }
        .admin-panel-body { padding:16px;display:flex;flex-direction:column;gap:14px; }
        .panel-field label { display:flex;align-items:center;gap:5px;font-size:11px;font-weight:600;color:#64748b;letter-spacing:.05em;text-transform:uppercase;margin-bottom:6px; }
        .panel-select { width:100%;background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:9px 12px;font-size:14px;color:#e2e8f0;outline:none;transition:border-color .15s;cursor:pointer; }
        .panel-select:focus { border-color:#378ADD; }
        .btn-update { width:100%;display:flex;align-items:center;justify-content:center;gap:7px;background:#185FA5;border:none;border-radius:8px;padding:10px;font-size:14px;font-weight:600;color:#fff;cursor:pointer;transition:background .15s; }
        .btn-update:hover { background:#0c447c; }
        .divider { border:none;border-top:1px solid #1e3a5f;margin:0; }
        .btn-delete-full { width:100%;display:flex;align-items:center;justify-content:center;gap:7px;background:transparent;border:1px solid #7f1d1d;border-radius:8px;padding:9px;font-size:13px;font-weight:600;color:#fca5a5;cursor:pointer;transition:background .15s; }
        .btn-delete-full:hover { background:#1f0d0d; }
        @media(max-width:900px){ .admin-layout{grid-template-columns:1fr;} .admin-panel{position:static;} }

        /* ── Panel info fields ─────────────────────────────── */
        .ticket-description { font-size:14px;color:#cbd5e1;line-height:1.6;white-space:pre-wrap;background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:12px; }
        .screenshot-container { border-radius:8px;border:1px solid #1e3a5f;overflow:hidden;background:#0a0f1e; }
        .screenshot-img { max-width:100%;height:auto;display:block;border-radius:6px;cursor:zoom-in; }
        ...
        /* OP image fills the bubble width for clear visibility */
        .op-attachment {
        ...
        .btn-remove-img { background:transparent;border:none;color:#f87171;cursor:pointer;font-size:18px;line-height:1;padding:0; }

        /* ── Thread toolbar ────────────────────────────────── */
        .thread-toolbar { display:flex;gap:6px;align-items:center;padding:10px 16px 0; }
        .btn-toolbar {
        ...
        .lightbox-close { position:absolute;top:18px;right:22px;font-size:28px;color:#fff;cursor:pointer;line-height:1;background:none;border:none; }
    </style>
</head>
<body>
...
<header class="topbar">
    ...
    <div class="topbar-right">
        ...
        <a href="${pageContext.request.contextPath}/admin-dashboard.jsp" class="btn-logout">
            <i class="ti ti-arrow-left"></i> All Tickets
        </a>
...
<main class="main-content">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">
                Raised by <strong><c:out value="${ticket.creatorName}"/></strong>
                &nbsp;·&nbsp;
    ...
    <div class="meta-row">
        <div class="meta-card">
            <div class="meta-label"><i class="ti ti-user"></i> Raised By</div>
            <div class="meta-value"><c:out value="${ticket.creatorName}"/></div>
        </div>
        <div class="meta-card">
    ...
    <div class="admin-layout">
        ...
        <div class="thread-card">
            ...
            <div class="thread-body" id="threadBody">
                ...
                <%-- Always shown first so the admin sees the user's full
                     original report, including any image attached at creation. --%>
                <div style="display:flex;flex-direction:column;align-items:flex-start;margin-bottom:4px;">
                    <div class="bubble-starter">
                        ...
                        <%-- Ticket image — strip any leading slash so contextPath+"/"+path is always valid --%>
                        <c:if test="${not empty ticket.imageLink}">
                            <c:set var="cleanImg" value="${fn:startsWith(ticket.imageLink,'/') ? fn:substring(ticket.imageLink,1,fn:length(ticket.imageLink)) : ticket.imageLink}"/>
                            <div class="op-attachment">
                                <img src="${pageContext.request.contextPath}/${cleanImg}"
                ...
                <%-- Empty state — only shown when no replies yet (OP bubble always shows above) --%>
                <c:if test="${empty messages}">
                    <div class="thread-empty">
                        <i class="ti ti-message-circle"></i>
                        <span>No replies yet. Start the conversation below.</span>
                    </div>
                </c:if>
            ...
            <div class="thread-footer">
                <div class="thread-toolbar">
                    <button type="button" class="btn-toolbar" onclick="document.getElementById('adminImageInput').click()">
                ...
                <div class="img-preview-wrap" id="adminImgPreviewWrap" style="margin:6px 16px 0;">
                    <img id="adminImgPreviewThumb" src="" alt="">
                    <span class="img-preview-name" id="adminImgPreviewName"></span>
                    <button type="button" class="btn-remove-img" onclick="clearAdminImageInput()">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                ...
                <form action="${pageContext.request.contextPath}/TicketControllerServlet"
                    ...
                    <input type="hidden" name="command"  value="ADMIN_SEND_MESSAGE">
                    <input type="hidden" name="ticketId" value="${ticket.id}">
                    <input type="file" name="image" id="adminImageInput"
                           accept="image/*" style="display:none"
                           onchange="previewAdminImage(this)">
                    <textarea name="message" class="msg-input"
                              placeholder="Reply to this ticket…" rows="3" required></textarea>
        ...
        <div class="admin-panel">
            <div class="admin-panel-header"><i class="ti ti-settings"></i> Ticket Controls</div>
            <div class="admin-panel-body">

                <div class="panel-field">
                    <label><i class="ti ti-notes"></i> Initial Description</label>
                    <div class="ticket-description">
                        <c:out value="${empty ticket.description ? 'No description provided.' : ticket.description}"/>
                    </div>
                </div>

                <%-- Ticket image in panel sidebar for quick reference --%>
                <c:if test="${not empty ticket.imageLink}">
                    <c:set var="cleanImg" value="${fn:startsWith(ticket.imageLink,'/') ? fn:substring(ticket.imageLink,1,fn:length(ticket.imageLink)) : ticket.imageLink}"/>
                    <div class="panel-field">
                        <label><i class="ti ti-photo"></i> User Screenshot</label>
                ...
                <%-- Status / Priority update — standard POST, no file upload needed --%>
                <form action="${pageContext.request.contextPath}/TicketControllerServlet" method="post">
                    <input type="hidden" name="command"  value="UPDATE_STATUS">
                    <input type="hidden" name="ticketId" value="${ticket.id}">
                    <div class="panel-field">
                        <label><i class="ti ti-refresh"></i> Status</label>
                    ...
                    <div class="panel-field">
                        <label><i class="ti ti-flag"></i> Priority</label>
                        <select name="priority" class="panel-select">
                            <option value="Low"    ${ticket.priority == 'Low'    ? 'selected' : ''}>Low</option>
                            <option value="Medium" ${ticket.priority == 'Medium' ? 'selected' : ''}>Medium</option>
                            <option value="High"   ${ticket.priority == 'High'   ? 'selected' : ''}>High</option>
                        </select>
                    </div>
                    <button type="submit" class="btn-update"><i class="ti ti-check"></i> Save Changes</button>
                </form>

                <hr class="divider">

                <form action="${pageContext.request.contextPath}/TicketControllerServlet" method="post"
                      onsubmit="return confirm('Permanently delete this ticket? Cannot be undone.')">
                    <input type="hidden" name="command"    value="DELETE_TICKET">
                    <input type="hidden" name="ticketId"   value="${ticket.id}">
                    <input type="hidden" name="redirectTo" value="admin">
                    <button type="submit" class="btn-delete-full"><i class="ti ti-trash"></i> Delete Ticket</button>
                </form>
...
<script>
    ...
    // ── Admin image attach preview ────────────────────────────────
    function previewAdminImage(input) {
        const wrap  = document.getElementById('adminImgPreviewWrap');
        const thumb = document.getElementById('adminImgPreviewThumb');
        const name  = document.getElementById('adminImgPreviewName');
    ...
    function clearAdminImageInput() {
        const input = document.getElementById('adminImageInput');
        input.value = '';
        document.getElementById('adminImgPreviewWrap').classList.remove('active');
        document.getElementById('adminImgPreviewThumb').src = '';
        document.getElementById('adminImgPreviewName').textContent = '';
    }
    ...
    // ── Browser push notifications: poll for new user replies (admin side) ──
    (function() {
        const TICKET_ID   = '${ticket.id}';
        const POLL_MS     = 15000;
        const STORAGE_KEY = 'admin_lastMsgCount_ticket_' + TICKET_ID;
        ...
        function sendNotif(title, body) {
            ...
            try {
                const n = new Notification(title, {
                    ...
                    tag: 'admin-ticket-reply-' + TICKET_ID,
                    renotify: true
                });
            ...
            pollTimer = setInterval(async () => {
                ...
                try {
                    ...
                    if (data.count > knownCount) {
                        ...
                        sessionStorage.setItem(STORAGE_KEY, String(knownCount));
                        sendNotif('HelpDesk — User Replied',
                            diff + ' new message' + (diff > 1 ? 's' : '') + ' on ticket #' + TICKET_ID + '.');
                        showToast('User replied on this ticket — reload to see the latest.', 'success');
                    }
                } catch(e) {}
            }, POLL_MS);
        }
